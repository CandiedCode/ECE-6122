...
program must: 1. Prompt the user to enter the number of readings (between 5 and 100) 2. Dynamically read that many double values from standard input 3. Implement the following functions that compute statistics using pass-by-reference 
...
void findExtrema(const double data[], int size, 
                 double& minVal, double& maxVal, int& minIndex,                   int& maxIndex); 4. Display all computed statistics with appropriate formatting (2 decimal places) 
Requirements: - Use a for loop to read input values - Use appropriate loops for statistical 
calculations - Validate that the number of readings is within the specified range using 
conditional statements - The const keyword must be used appropriately to prevent 
modification of input arrays
