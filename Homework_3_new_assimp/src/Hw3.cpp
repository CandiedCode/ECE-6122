...
auto loadModels(const char *executablePath) -> std::vector<SceneObject>
{
    ...
    auto bullseye_path = resolveFilePath("./assets/toystory/toy_story_bullseye.glb", executablePath);
    if (!bullseye_path.has_value())
    {
        std::cerr << "Error: Could not find bullseye model at './assets/toystory/toy_story_bullseye.glb' or relative to executable\n";
        exit(EXIT_FAILURE);
    }
    auto *model_bullseye = new Model(bullseye_path.value()); // NOLINT(cppcoreguidelines-owning-memory)

    auto buzz_path = resolveFilePath("./assets/toystory/buzz_lightyear.glb", executablePath);
    if (!buzz_path.has_value())
    {
        std::cerr << "Error: Could not find buzz model at './assets/toystory/buzz_lightyear.glb' or relative to executable\n";
        exit(EXIT_FAILURE);
    }
    auto *model_buzz = new Model(buzz_path.value()); // NOLINT(cppcoreguidelines-owning-memory)

    auto woody_path = resolveFilePath("./assets/toystory/woody.glb", executablePath);
    if (!woody_path.has_value())
    {
        std::cerr << "Error: Could not find woody model at './assets/toystory/woody.glb' or relative to executable\n";
        exit(EXIT_FAILURE);
    }
    auto *model_woody = new Model(woody_path.value()); // NOLINT(cppcoreguidelines-owning-memory)

    auto slinky_path = resolveFilePath("./assets/toystory/slinky_dog_rigged.glb", executablePath);
    if (!slinky_path.has_value())
    {
        std::cerr << "Error: Could not find slinky model at './assets/toystory/slinky_dog_rigged.glb' or relative to executable\n";
        exit(EXIT_FAILURE);
    }
    auto *model_slinky = new Model(slinky_path.value()); // NOLINT(cppcoreguidelines-owning-memory)

    auto hamm_path = resolveFilePath("./assets/toystory/hamm.glb", executablePath);
    if (!hamm_path.has_value())
    {
        std::cerr << "Error: Could not find hamm model at './assets/toystory/hamm.glb' or relative to executable\n";
        exit(EXIT_FAILURE);
    }
