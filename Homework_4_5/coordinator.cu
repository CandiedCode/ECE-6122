...
{
    if (argc < 4) {
        std::cerr << "Usage: ./build/coordinator <image_width> <image_height>"
                     " <tile_size> [port]\n";
        return 1;
    }
    ...
    const unsigned short port = (argc >= 5)
                                    ? static_cast<unsigned short>(std::stoi(argv[4]))
                                    : 54000;
    ...
    scene.cam_origin[0]  =  0.f;  scene.cam_origin[1] = 1.f;  scene.cam_origin[2] = 5.f;
    scene.cam_lookat[0]  =  0.f;  scene.cam_lookat[1] = 0.f;  scene.cam_lookat[2] = 0.f;
    scene.fov_deg        = 60.f;
    ...
    std::cout << "[coord] Listening on port " << port << "\n"
              << "[coord] Waiting " << REGISTRATION_WAIT_MS
              << " ms for worker registrations...\n";
    ...
    for (int ty = 0; ty < tiles_y; ++ty) {
        for (int tx = 0; tx < tiles_x; ++tx) {
            ...
            ti.order.tile_w   = static_cast<uint16_t>(
                std::min((int)tile_size, (int)image_w - tx * (int)tile_size));
            ti.order.tile_h   = static_cast<uint16_t>(
                std::min((int)tile_size, (int)image_h - ty * (int)tile_size));
            pending.push(tid);
        }
    }
    std::cout << "[coord] " << total_tiles << " tiles queued ("
              << tiles_x << " x " << tiles_y << ").\n";
    ...
    sf::RenderWindow window(
        sf::VideoMode(image_w, image_h),
        "Ray Tracer  \u2014  0 / " + std::to_string(total_tiles) + " tiles complete");
    ...
    while (tiles_done < total_tiles) {
        ...
        while (window.pollEvent(ev))
            if (ev.type == sf::Event::Closed) { window.close(); goto shutdown; }
    ...
    // ── Render complete (provided) ────────────────────────────────────────
    {
        ...
        std::cout << "[coord] Render complete in "
                  << ms / 1000 << "." << (ms % 1000) / 10 << " s.\n";
    }
